...
- [针对 KVM 的额外说明](@/aosc-os/installation/amd64-notes-kvm.zh.md)
- [针对 Bay Trail/Cherry Trail 的额外说明](@/aosc-os/installation/amd64-notes-trails.zh.md)
- [针对 software RAID 的额外说明](@/aosc-os/installation/amd64-notes-softraid.zh.md)
...
所有的 AMD64/x86_64 的 .tar 压缩包都是通用的。您要做的就是按您的喜好和需求选择他们。

> 注意：您在选择 .tar 压缩包的时候还要考虑您的设备是否支持，关于这部分的信息请参考 [AMD64/x86_64 system requirements](@/aosc-os/installation/amd64-notes-sysreq.md)。
...
> **警告：请确认您下载的是 AMD64 的版本，否则您将无法进入 AOSC OS 的 Chroot 环境！**

> 注意： 在使用 VMware 的过程里您可能会无法连接网络。

先准备 Live 环境，在这里使用 `dd` 来写入镜像。如果您使用的是 Windows，可以使用 [Rufus](https://rufus.akeo.ie/) 或 [Win32DiskImager](https://sourceforge.net/projects/win32diskimager/)。
...
- `nameofimage.iso` 是您下载的 GParted Live 的文件位置。
- `/dev/sdX` 是您的 USB 储存设备的路径。

在完成后，请启动到 Live 环境。

# 准备分区

在 AMD64/x86_64 架构下，AOSC OS 支持 GUID（用于 EFI 引导）和 MBR（用于传统 BIOS 引导）两种分区表。如果您将要与微软的 Windows，苹果的 macOS 或其他 Linux 发行版组成多系统，他们大多是使用 GUID 分区表的，而如果需要在老设备上进行安装，请选择 MBR 引导。

GParted Live 环境里的 GParted 使用起来非常简单。如果对如何使用有疑问，请参考：[GParted Manual](http://gparted.org/display-doc.php?name=help-manual)。

## 附加提示

- 如果您计划在多个分区上安装 AOSC OS，请在重启进入 AOSC OS 之前确认您创建了正确的 `/etc/fstab`，具体的方法将会在稍后说明。
- 如果您计划把 ESP（EFI 系统分区）作为 `/boot` 挂载，在升级系统时可能需要特别的注意，具体细节将会在稍后说明。

# 解压 Tarball

在分区设置完成后，已经可以开始解压您已经下载好的 AOSC OS 系统 Tarball。
在开始解压之前请把您的系统分区挂载到合适的路径。提示
在这里假设您要把 AOSC OS 安装到 `/dev/sda2`：

```
# mount -v /dev/sda2 /mnt
```

如果您需要把 `/dev/sda3` 作为 `/home`：
...
如果您无法进入 Chroot 环境，很可能是因为您下载了错误架构的 Tarball（这个在之前已经 **加粗** 强调过了……）。
...
> 注意：您需要 GRUB 2.02（`grub` 版本 `2:2.0.2`）来将 NVMe-based 设备设定为启动驱动器。

> 注意：下面的所有命令都应该在 chroot 环境中执行。

## EFI 引导

在安装 GRUB 之前，请先挂载 ESP 分区，在本文里把 `/dev/sda1` 挂载到 `/efi` 作为示例（请根据您的实际情况修改命令里的路径）：
...
所有的 Tarballs 都不带有默认账户且 `root` 账户默认被禁用（您仍然可以用 `sudo` 来获取超级用户权限），您需要在重启进入 AOSC OS 之前创建一个账户。
...
尽管不建议这么做，您仍可以通过给 `root` 账户设置密码来启用它：
...
AOSC 在默认情况下启用了所有 UTF-8 编码的 Locale 。在一些情况下，如果您想要启用或禁用某些 Locale ，请编辑 `/etc/locale.gen`，并以 `root` 权限执行 `locale-gen`（这可能需要花费大量的时间）。
...
> 注意：在重启计算机后，您还可以使用 `localectl` 命令来做这件事：
...
> 注意：在重启计算机后，您还可以使用 `hostnamectl` 命令来做这件事：
