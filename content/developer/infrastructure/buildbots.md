...
| **lpi-2** | ~~23456~~ | Loongson 2K-1000 @ 1GHz (Loongson Pi 2) | 2GiB | 3850s (`-j3`) | _Junde Yhi_ | Sponsored by [windows1089](https://web.archive.org/web/20210228060233/http://www.openloongson.org/?4655) |
...
| **Resonance** | 23541 | Loongson 3B-4000 (R4) @ 1.8GHz (LX-2510) | 32GiB | 265s (`-j9`, 2.31) | _Kexy Biscuit_ | Community crowdsourced; Scratch disk at `/buildroots`, create own Ciel workspace; repo mirror at `http://localhost/debs/`, BuildKit mirror at `http://localhost/aosc-os/os-amd64/buildkit/` |
...
| **Kirin** | N/A | HUAWEI Kirin 9006C (8) @ 2.045GHz | 8GiB | 143.5s (`-j`, 2.34) | _Rick Liu_ | Scratch disk at `/buildroots`, create own Ciel workspace |
...
| **marianne** | 26055 | SiFive FU740 @ 1.4GHz (SiFive HiFive Unmatched) | 16GiB | | _Icenowy Zheng_ | Scratch at `/buildroots` is on NBD and needs to be mounted manually by nbd-mount.sh in /root (currently using the `/buildroots` from **lorenz**), behind GFW, a HTTP proxy is available at http://dedue:8118, device sponsored by PLCT Lab |
| **leonie** | 26056 | SiFive FU740 @ 1.4GHz (SiFive HiFive Unmatched) | 16GiB | | _Icenowy Zheng_ | Scratch at `/buildroots` is on NVMe disk (maybe buggy, investigation needed), behind GFW, a HTTP proxy is available at http://dedue:8118, device sponsored by PLCT Lab |

## **LoongArch** (27001-28000)

| Name | Port | CPU | Memory | Speed | Maintainer | Note |
|-----------|-----------|-----------|-----------|-----------|-----------|-----------|
| **Yukoaioi** | 27001 | Quad core @ 2.5GHz (Loongson 3A5000) | 16GiB | 183s (`-j5`, Loongnix [2.28-10.lnd.26](http://pkg.loongnix.cn:8080/loongnix/pool/main/g/glibc/glibc-source_2.28-10.lnd.26_all.deb)) | _Mag Mell_ | direct access with `ssh -p 9876 root@athome.miraclemilk.me` |
