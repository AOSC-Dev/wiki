...
Next, we will initialise and configure a Ciel workspace. **Note: Ciel needs to be run as `root`.** You may use `sudo -i` to gain elevated permission, then the root shell will be entered, and the `~` will point to `/root`. You can also sdd `sudo` before each command (the corresponding directory of `~` will not change).
