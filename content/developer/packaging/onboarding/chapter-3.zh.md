...
[chapter-4]: @/developer/packaging/onboarding/chapter-4.zh.md
...
上一章讲到，编译一般指将文本形式的源代码由编译器读取并生成二进制文件的步骤。而在发行版维护中存在另一个概念。

发行版维护中所涉及的软件项目不会只由单个或多个文件组成。并且，这些软件项目可能还包含使用手册、API 文档、图标、字符串翻译等内容，这些内容会随着编译的二进制安装进系统中。我们将整个软件项目的源码编译、文档生成、图片转换（矢量图栅格化）、翻译编目生成等涵盖整个项目最终产出的过程称为**构建 (Build)**。构建有时也指较大规模源码的编译过程，如 Linux 内核构建。

在构建多个源码组成的项目时，一般使用 `-c` 参数分别将每个源码文件编译为二进制可重定位文件。所有源码编译完毕后，再调用编译器将生成的 `.o` 文件链接成最终的可执行文件或共享库。
