...
我们已经看到，对于许多包，Autobuild 可以自动确定源代码树中使用的构建系统，然后相应地生成和执行构建脚本。但是有许多（复杂的或原始的）程序需要更多步骤来构建和安装，或者可能需要特定的构建参数和编译参数。

现在我们将介绍如何在 AOSC OS 的构建系统中处理这些问题。

## 手动选择不同的构建系统

有时 Autobuild 可能对选择错误的构建系统，并可能会导致构建失败。比如当一个项目有多个构建系统可用时，Autobuild 可能不会选择（对于构建时间或可靠性来说）最好的一个。
...
目前，支持这些构建类型：
  ...
  - `autosetup`：通常用于基于 Autosetup 的源代码树，在源根目录中具有可用的 auto.def 和 configure 文件以及 autosetup 目录。
  - `cmake`：用于基于 CMake 的源代码树，生成并执行 Makefile，在源根目录中具有可用的 CMakeLists.txt。
  ...
  - `perl`：用于标准 CPAN 源代码树。
  - `pep517`：用于基于 PEP517 的 Python 源代码树，需要存在 pyproject.toml 文件。
  - `python`：用于基于 setup.py 的 Python 源代码树，需要存在 setup.py 文件（如果可能，更建议使用 `pep517`）。
  - `qtproj`：用于源代码树中包含 .pro 文件的 Qt 项目。
  - `ruby`：用于 RubyGems 源代码树。
  - `rust`：用于 Cargo 源代码树（通常用于 Rust 源代码）。
  - `waf`：用于基于 Waf 的源代码树，Autobuild 检测源代码树中的 waf 文件或脚本。

## 自定义构建系统/编译器参数

Autobuild 集成了一系列最佳构建参数。但是，有时这些参数与软件不完全兼容，可能会引起问题。在这种情况下，请查看 [default-flags.sh](https://github.com/AOSC-Dev/Autobuild/blob/master/lib/default-flags.sh) 和 [default-defines.sh](https://github.com/AOSC-Dev/Autobuild/blob/master/lib/default-defines.sh) 中的默认参数，并根据需要覆盖对应的参数。

其中一个突出的问题是 LTO（链接时优化）。这种技术可以提高运行时的效率，并减小二进制文件的大小，但目前启用 LTO 可能导致构建失败（这种情况的数量在不断减少），并且在构建时消耗大量内存。Autobuild 默认启用 LTO 以提高性能（有时也是为了减小二进制文件的大小）。但如果您遇到与 LTO 相关的问题，可以通过在 `autobuild/defines` 中添加 `NOLTO=1` 来禁用 LTO。

## 自定义构建脚本

在某些情况下，软件使用特殊的构建系统（或者根本不需要构建系统，如预构建的二进制文件）。在这种情况下，您可以通过使用 Bash 编写构建脚本来控制构建过程。

构建脚本位于 `autobuild/build`。如果存在该脚本，构建类型将被锁定为 `self`（除非通过定义了另一个 `ABTYPE=` 覆盖了构建类型），这意味着 Autobuild 不会尝试确定构建系统并执行其集成的构建脚本，而是简单地执行这个脚本。

这个脚本应该看起来非常类似于手动编译程序的过程。但是有一个关键的区别：您 **不** 应该将编译后的程序安装到系统根目录。而是应该安装在 `$PKGDIR` 中，稍后 Autobuild 将根据此目录中的文件创建 deb 包。例如，如果编译后的二进制文件在构建目录的根目录中称为 `hugo`，那么您应该将其安装到软件包的 `bin` 目录中，如下所示：
...
有时 Autobuild 可以很好地处理构建过程，但最终产出的包可能需要一些额外的调整（例如：man 页面的目录不对，需要将 shell 自动补全脚本复制到 `$PKGDIR` 等）。在这种情况下，我们使用 `autobuild/beyond` 脚本。与 `autobuild/build` 类似，这个脚本作为纯 Bash 脚本在构建后执行。
...
为了缓解这个问题，我们引入了 `autobuild/patches/series` 文件。这个文件包含补丁名称的有序列表（每行一个文件名）。如果存在这个文件，Autobuild 将按照列表中指定的顺序应用补丁。
