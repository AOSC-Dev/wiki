...
根据 [E432](@/developer/system/qa-issue-codes.md#class-4-dependencies) 的规定，所有 ELF 级别的直接依赖项都应该被写进 `$PKGDEP`，这意味着你还需要为 `$PKGDEP` 补上一个 `dbus`。
...
在为 AOSC OS 打包时，请遵循我们的 [发行版特性清单](@/system/is-aosc-os-right-for-me.md)。下表列出了打包者需要注意的一些事项。 
...
| QA 问题 | `$PKGNAME: ... ($ISSUECODE)` | `psiconv: rebuild for imagemagick (E431)`，请阅读 [这个清单](@/developer/system/qa-issue-codes.md) 了解各个 `$ISSUECODE` 对应的含义 |
